 ...
 *
 * 画布等元素级状态以「状态条目追加日志」存入 Y.Doc（见协作会话旁的 canvasDoc.ts）：每次写入追加一条
 * 完整快照，Yjs 只负责日志复制，合并结果由本模块的纯函数给出，不引入第二份真相。
 * 四要素在 Y.Doc 侧的落实：
 * - 版本号：元素自己的单调计数器，本地写入取「文档当前胜者版本 + 1」；
 * - 随机决胜：写入方 Y.Doc.clientID 的定宽十进制（字典序等价数值序），同版本时大者胜；
 * - 删除：墓碑状态（deleted=true）留在日志中阻止旧更新复活，同步轮次后清除；
 * - 排序：每条状态带分数，orderVisibleElements 给出多端一致的可见顺序。
 *
 * 合并规则（交换、结合、幂等）：
 * 1. 版本高者胜；
 * 2. 版本相同：墓碑胜更新；
