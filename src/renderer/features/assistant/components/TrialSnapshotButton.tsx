...
/** AI 试错回滚入口：展示最近一次试错快照，一键回到任务开始前。 */
...
const TrialSnapshotButton: React.FC<TrialSnapshotButtonProps> = ({ projectId, onUpdate }) => {
  ...
  const rollback = () => {
    const chapters = aiTrialSnapshots.rollback(latest.id);
  ...
  return (
    <div className="flex items-center justify-between gap-2 border-b border-border bg-muted/20 px-4 py-1.5">
      <span className="min-w-0 truncate text-2xs text-muted-foreground">{t('trial.label', { label: latest.label })}</span>
      {confirming ? (
