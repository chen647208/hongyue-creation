...
8. **[08 路线图](08-roadmap.md)** — M0–M5 工作包分解、退出标准、横切策略、风险登记（时点排期记录）
...
11. **[11 会话记忆与压缩](11-chat-memory.md)** — 助手多轮记忆、超长压缩、重试按钮
12. **[12 写作管理](12-writing-manage.md)** — 章节状态标记、批量操作、卡片视图、历史 diff
13. **[13 导出矩阵](13-export-matrix.md)** — ePub / DOCX 复用 build 管线的最小打包器
14. **[14 助手进阶](14-assistant-advanced.md)** — 图片附件、计划模式、后台执行、MCP 客户端
15. **[15 系统补齐](15-system-roundup.md)** — 标签分组、托盘自启、代理、崩溃上报（键位自定义见 50 篇）
16. **[16 新功能补齐](16-new-features.md)** — 封面导出、人物卡导出、文档附件、语音
17. **[17 行业基线补齐](17-industry-gaps.md)** — 数据安全、合规、AI 可靠性、无障碍的缺口与验收
18. **[18 规范化重构](18-standardization.md)** — 组件化、代码标准、插件化的总案与批次规划
...
19. **[27 IPC 信任边界加固](27-ipc-hardening.md)** — SQL 语义通道、文件 IPC 收敛与 CSP 决策
20. **[29 通用创作平台](29-general-creation-platform.md)** — 实体类型与字段自定义、双轴时间线、多视图、类型模板、可开关界面与分期改造计划
...
27. **[36 数据同步与冲突](36-data-sync-and-conflict.md)** — 同步包落库修复、WebDAV/S3 传输、冲突与恢复提示
28. **[37 AI 上下文注入与可信检索](37-ai-context-and-grounding.md)** — 关键词命中注入、预算与溯源逐字校验
29. **[38 修订、批注与关联](38-revision-annotation-and-linking.md)** — 逐处接受/拒绝、行内批注、反向引用、登场章节与章纲提取
30. **[39 编译与导出](39-compile-and-export.md)** — 编译目标与模板、ODT、分镜表与口播输出
31. **[40 生态与运行时能力](40-ecosystem-and-runtime.md)** — 插件分发、多助手会话、MCP 资源面、联网搜索/翻译、本地推理
32. **[41 非虚构与引用](41-nonfiction-and-reference.md)** — 引文/脚注/参考文献、对照视图
33. **[42 分支叙事与绘本](42-branching-and-picturebook.md)** — 分支与变量、完整性校验、图位与页
34. **[43 无障碍与国际化品质](43-accessibility-and-i18n.md)** — 全量 axe、对比度 AA、中英对齐校验
35. **[44 创作范式地图与跨域能力](44-paradigms-and-cross-domain.md)** — 各领域范式地图、跨域可迁移机制、本仓落点与分档
36. **[45 查询、块引用与素材隔离](45-query-blocks-and-materials.md)** — 保存的动态视图、块级引用与嵌入、素材隔离
37. **[46 时间线深化与版本](46-timeline-and-versioning.md)** — 非破坏操作、草稿矩阵与成稿轨道、试错快照与正式历史
38. **[47 跨域视图与脚本层](47-cross-domain-views-and-scripting.md)** — 图 DSL 视图、可编程画布、图表视图、宏/脚本层
39. **[48 后续任务总表](48-backlog.md)** — 34–47 的缺口按依赖与收益排序、分阶段执行
40. **[49 可执行插件描述符协议](49-executable-plugin-protocol.md)** — 渲染器/脚本描述符、能力白名单、同步渲染契约与沙箱边界
41. **[50 键位自定义与缩放接管](50-keybindings-and-zoom.md)** — 缩放单口径（界面字号）、快捷键命令目录与用户改键、localStore 持久化与冲突/保留提示
42. **[51 安全授权收口](51-security-authorization.md)** — 信任决策写路径经用户确认、签名两级语义、沙箱限额钳制、网关 Key 域绑定
