...
落地边界：协议层与宿主门序（激活 → 注册 → 纯同步约束 → 能力回查 → 入口存在 → 输入 schema → 同步端口 → 输出字符串 + output schema）落定，端口类型 `RendererExecutionPort` 由宿主注入，缺省即拒绝（fail-closed）。生产端口在渲染进程内同步跑 QuickJS：`preheat` 异步加载 `quickjs-emscripten`（RELEASE_SYNC 变体）并在独立 runtime/context 编译入口，`render` 同步调用导出函数；遵守 `PLUGIN_RENDERER_TIMEOUT_MS`（毫秒级，禁秒级长任务），并设内存上限与输出字节上限，超限中断返回结构化错误。隔离取舍不可接受时回落选型 A。
...
- 脚本事件触发：宿主声明的事件挂点单源在 `core/plugin/execution.ts` 的 `HOST_SCRIPT_EVENTS`（`project.open` / `chapter.open` / `chapter.save`）；`PluginHost.emit(event, payload)` 遍历 `on === event` 且所属插件已激活的脚本逐一 `runScript`，未知事件直接返回，单脚本失败经 `onScriptEventError` 上报、不阻断其余脚本与主流程。应用层触发点：`projectStore.setActiveProject` → `project.open`；`WritingEditor` 活动章切换 → `chapter.open`；`persistenceBridge.doFlush` 落盘成功后按引用变化的章节 → `chapter.save`。触发异步、非阻塞；`hasEventSubscribers` 在无订阅时跳过载荷计算，缺省零开销。
- 沙箱执行（渲染器面）：`PluginHost.runRenderer(pluginId, rendererId, input)` 走同步门序（激活 → 注册 → 纯同步约束 → 能力回查 → 入口存在 → 输入 schema → 同步端口 → 输出字符串 + output schema），经注入的 `RendererExecutionPort`（`preheat` + `render`，毫秒级超时）同步执行；端口缺省即拒绝。生产端口 `renderer/shared/services/rendererExecutionPort.ts` 在激活时异步预热入口（`quickjs-emscripten` 动态 `import` 懒加载，无逻辑插件时不进主包），`render` 同步调用；内存、墙钟、输出三项超限中断并返回结构化错误，超时/内存错误后同入口持续拒绝（fail-closed）；缓存键含 `pluginId`，停用逆序释放。导出构建管线消费该端口：`core/build/pipeline.ts` 声明 `PluginRendererPort`（只读 `list` + 同步 `render`）与 `BuildOptions` 注入项，`renderer/shared/services/buildRendererPort.ts` 把宿主门控后的同步执行适配成该端口并登记当前宿主；显式 `profile.renderer` 选择优先于同格式内置渲染器，同格式有内置且未指定时保持内置，端口缺省逐字回落内置（零回归），插件渲染失败抛可读错误不静默回落。
- 能力调用通道与 `tool:propose`：`resolveCapabilities` / `allowedToolNames` 只放行描述符声明且过权限回查的能力；`runScript` 把能力名交执行端口，工具调用按白名单裁决，提议经注入的 `ToolProposalPort` 进审批管线，脚本无直接写路径。生产端口 `createCapabilityDispatchPort`（`core/plugin/capabilityDispatch.ts`）+ `pluginCapabilityPort.ts`：`read:<域>` 走既有 `repository` 查询，`write:<域>` 先经 `ApprovalRouter` 弹批再交既有提案执行器，`net` 走主进程 `pluginNet`（netGate），`ai` 走既有 AI 网关；`net`/`ai` 回执经 `fenceUntrusted` 围栏，结果经 `ScriptRunOutcome.toolResults` 回给调用方。未声明能力不可见、越权拒绝，端口缺省即拒绝提议。助手层契约（审批 `broker`、提案执行器、当前模型读取器）经 `CapabilityHostBindings` 由应用层装配处（`features/assistant/services/aiRuntime.ts`）随 `bootstrapPlugins` 注入；`pluginService` 与 `pluginCapabilityPort` 只接受注入，不反向 import 助手层，注入项缺席即对应能力 fail-closed。
